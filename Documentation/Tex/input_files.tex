...
%
Cassandra will then use \texttt{dee} as prefix for all output files created.
...
Options are ``cut'', ``cut\_tail'', ``cut\_switch'' and
``cut\_shift". Refer to Chapter~\ref{Chapter:Force Field} for the
functional forms.
...
be smooothly taken to zero at 14.0 \AA. Obviously, $r_{on} < r_{off}$ or \emph{Real(i,3)} $<$ \emph{Real(i,4)}.
%
\\ \\
...
{\it Real(1) Real(2) .... } \\ \\
%
Sets the temperature in Kelvin {\it Real(1) Real(2) ... } for simulation
boxes 1, 2, ... 
%%%%%%%%%%%%%%%%
%
% Pressure_Info
...
{\it Real(1) Real(2) ... } \\
%
Specifies the pressure in bar {\it Real(1) Real(2) ...} for simulation
boxes 1, 2, ... Note that if the simulation type does not require an
input pressure (for example, an NVT simulation), this command will be
ignored.  
%%%%%%%%%%%%%%%%
%
% Fugacity_Info
%
%%%%%%%%%%%%%%%%
\subsection{Fugacity}\label{sec:Fugacity_Info}
{\bf \# Fugacity\_Info} \\
{\it Real(1) Real(2) ...} \\ \\
%
Specifies the fugacities in bar {\it Real(1) Real(2) ...} of {\bf
  insertable} species in the order in which they appear in the
Molecule\_Files section.  The fugacity will be arbitrarily set to zero
for species that cannot be swapped or exchanged with a reservoir. This
option is used with grand canonical ensemble simulations. It will be
ignored for all other simulation types.
%%%%%%%%%%%%%%%%%%
%
...
{\it Real(1) Real(2) ....} \\ \\
%
This keyword is only used for grand canonical simulations and may be
used instead of fugacity. It sets the
chemical potential of the insertable species in the order in which
they appear in the \texttt{Molecule\_Files} section. The chemical potentials
will be set arbitrarily to zero for species that cannot be swapped or
exchanged with a reservoir. Note that the de Broglie wavelength of the
species in each box is automatically calculated and used in the
acceptance rules. Units of chemical potential are kJ/mol. \textbf{Note: if you
specify BOTH a fugacity AND a chemical potential, Cassandra will use
whichever one appears first in the input file and will ignore the
other keyword. We recommend that you do not list both keywords in any
input file.}
...
of the time. For box 1, the maximum displacement will be 2.0 \AA\ for
...
\\ 
%
This will tell Cassandra to perform rotational moves 25\% of the time. For box 1,
...
30.0 10.0 0.0}
\\ \\
This will tell Cassandra to attempt rotational move 25\% of the time. 
...
0.0 0.7 0.3 } \\ \\
%
This will tell Cassandra to attempt regrowth move 30\% of the time. The relative probabilities of
...
time. The box volume would be changed by random amounts ranging from
...
%
This will tell Cassandra to attempt volume moves 2\% of the
time. The first box volume (assumed here to be smaller and of higher
...
bolder or more conservative moves. Note that if the \# Run\_Type is set
to 'Equilibration', Cassandra will attempt to optimize the boldness of
moves to achieve about 50\% acceptance rates. 
...
reservoir} \\ \\ 
%
This will tell Cassandra to attempt insertions 10\% of the time and
...
none} \\ \\ 
%
This will tell Cassandra to attempt insertions 10\% of the time. Only
...
%
This will tell Cassandra to attempt swap moves 10\% of the time. Attempts will be made to transfer species 1 and 2 between available boxes while molecules of species 3 will remain
...
{\it Real(1) Real(2)} \\ \\
%
This keyword is used when flip moves are to be attempted to sample bond angles and dihedral angles in a ring fragment. For more details on this move, see our publication \cite{Shah:2011}. The relative probability of attempting a flip move is 
specified by {\it Real(1)} while the maximum angular displacement in degrees for the move is given by {\it Real(2)}. For example, if the flip is to be attempted 30\% of the 
time and the maximum angular displacement for the move is 20\degree~specify the following: \\ \\
%
\texttt{
\# Prob\_Ring \\
0.30 20.0 \\ \\}
...
%
%%%%%%%%%%%%%%%%
\subsection{Frequency}\label{sec:Frequency_Info}
{\bf \# Frequency\_Info} \\
{\it freq\_type  Character(1)} \\
{\it Nthermofreq} or {\em thermofreq} {\it Integer(2)} \\
{\it Ncoordfreq} or {\em coordfreq} {\it Integer(3)} \\
{\it MCsteps} or {\em Stop} {\it Integer(4)} \\ 
{\bf \# Done\_Frequency\_Info} \\ \\
%
...
data is output. If {\it Character(1)} is to `Timed', then the simulation stops after the specified 
time in minutes. The format for this option is given below: \\ \\
%
{\bf freq\_type    Timed} \\
thermofreq       {\it Integer(2) }\\
coordfreq         {\it Integer(3)} \\
Stop                  {\it Integer(4)} \\ \\
%
...
coordfreq       10 \\
Stop                 60 \\ 
\# Done\_Frequency\_Info \\ \\}
%
Note that similar to \texttt{\# Move\_Probabilities} section, the end of the frequency section 
always includes the \\ \texttt{\#~Done\_Frequency\_Info} line. \\ \\
%
Simulations can also be run for a given number of MC steps. To enable this feature, 
{\it Character(1)} is set to `none'. Additional information is required and is given in the
following format: \\ \\
%
{\bf freq\_type    none} \\
Nthermofreq       {\it Integer(2) }\\
Ncoordfreq         {\it Integer(3)} \\
...
\texttt{Nmols}: Number of mols of species \\
