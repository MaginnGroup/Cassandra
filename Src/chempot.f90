...
SUBROUTINE Chempot(this_box,is)
  ...
  REAL(DP) :: P_forward, this_lambda, nrg_ring_frag_out
  ...
  IF(species_list(is)%fragment) THEN
     ...
     CALL Build_Molecule(alive,is,this_box,frag_order,this_lambda,anchor_dummy,P_forward, nrg_ring_frag_out, cbmc_overlap)
