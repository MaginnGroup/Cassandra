...
SUBROUTINE Identity_Switch
 ...
 ! Variables added for l_pair_nrg and reciprocal k vector storage
  ...
  !rotation variables
  REAL(DP) :: psi, phi, theta
  REAL(DP) :: psi1, psi2, psi3, sum_boltzmann
  INTEGER :: rotations, axis, i, selected_rot
  ...
  !ROTATION!
  !***************************************************************************!
  !REAL(DP), DIMENSION(:,:), ALLOCATABLE :: rotation_results

  LOGICAL, DIMENSION(:), ALLOCATABLE :: rotation_overlap
  ...
  DO i = 1, rotations
     IF(species_list(is)%linear) THEN
        ...
        rotation_results(i, 1) = theta
        rotation_results(i, 2) = phi
        rotation_results(i, 3) = psi
        CALL Rotate_Molecule_Eulerian(is, i_alive, theta, phi, psi)
      ELSE
        ...
        psi3 = 0.0_DP
        IF ( axis == 1 ) THEN
          psi1 = ( 2.0_DP * rranf() - 1.0_DP) * PI
        ELSE IF ( axis == 2 ) THEN
          psi2 = ( 2.0_DP * rranf() - 1.0_DP) * PI
        ELSE
          psi3 = ( 2.0_DP * rranf() - 1.0_DP) * PI
        END IF
        rotation_results(i, 1) = psi1
        rotation_results(i, 2) = psi2
        rotation_results(i, 3) = psi3
        CALL Rotate_Molecule(is, i_alive, psi1, psi2, psi3)
     END IF
     ...
     IF (inter_overlap) THEN
         ...
         rotation_results(i, 4) = 0.0
     ELSE
        !write (*, *) "No overlap found!"
        rotation_results(i, 4) = 1.0
        dE = 0.0_DP
        ...
        rotation_results(i, 5) = dE
        !TODO: Make well behaved numerically!
        rotation_results(i, 6) = DEXP(-dE)
    END IF

  END DO

  !Find the sum of the boltzmann factors
  sum_boltzmann = 0.0_DP
  DO i = 1, rotations
    IF (rotation_results(i, 4) == 1.0) THEN
      sum_boltzmann = sum_boltzmann + rotation_results(i, 6)
    END IF
  END DO

  !Calculate ratio of boltzmann factors
  DO i = 1, rotations
    IF (rotation_results(i, 4) == 1.0) THEN
      rotation_results(i, 7) = rotation_results(i, 6) / sum_boltzmann
    ELSE
      rotation_results(i, 7) = 0.0_DP
    END IF
  ...
  randno = rranf()
  DO i = 1, rotations
    IF (randno <= rotation_results(i, 7)) THEN
        !i will represent the selected rotation
        selected_rot = i
        EXIT
    ELSE
        randno = randno - rotation_results(i, 7)
    END IF
  END DO

  !Reverse back to the selected rotation
  DO i = rotations, (selected_rot + 1), -1
     IF(species_list(is)%linear) THEN
        theta = -1 * rotation_results(i, 1)
        phi = -1 * rotation_results(i, 2)
        psi = -1 * rotation_results(i, 3)

        CALL Rotate_Molecule_Eulerian(is, i_alive, theta, phi, psi)
      ELSE
        psi1 = -1 * rotation_results(i, 1)
        psi2 = -1 * rotation_results(i, 2)
        psi3 = -1 * rotation_results(i, 3)
        CALL Rotate_Molecule(is, i_alive, psi1, psi2, psi3)
     END IF
  END DO
  ...
  !Modified from the version in move_rotate.f90
  !now explicitly asks for molecule details (im, is) and the angles to rotate
  SUBROUTINE Rotate_Molecule(is, lm, psi1, psi2, psi3)

    IMPLICIT NONE
    INTEGER, INTENT(IN)  :: lm, is
    REAL(DP), INTENT(IN) :: psi1, psi2, psi3

    INTEGER :: ia
    REAL(DP) :: cospsi1, cospsi2, cospsi3
  ...
  SUBROUTINE Rotate_Molecule_Eulerian(is, lm, theta, phi, psi)
    ...
    INTEGER, INTENT(IN) :: is, lm

    REAL(DP) :: rot11, rot12, rot13, rot21, rot22, rot23
